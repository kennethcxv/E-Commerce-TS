...
const Checkout = () => {
  ...
  return (
    ...
    <Box sx={{ backgroundColor: "#e7e5e4", height: "100%", py: 4, display:"flex" }}>
      <form>
        <Box
          sx={{
            borderRadius: 2,
        ...
        <Box>
          <Typography sx={{ fontSize: 17, fontWeight:600 }}> PAYMENT INFO</Typography>
          ...
          <Box sx={{display:"grid", gridTemplateColumns:"1fr 1fr", width:"65%", }}>
          <TextField label="Month" sx={{mr:2, backgroundColor: grey[200] }}/>
...
>
        {addToCart.map((item) => {
          return(
            <Box sx={{mb:4,width: "100%",alignItems: "center",display:"flex"}}>

            <Box sx={{display:"grid",gridTemplateColumns: "130px 1fr auto",
 }}>
            <img src={item.image} className="w-64 h-46 " />
            <Typography sx={{ml:4,mt:2, width:"70%"}}>{item.title}</Typography>
            <Typography sx={{mt:2, mr:"auto",width:"120px"}}>${item.price * item.quantity} USD</Typography>
            </Box>
            </Box>
          )
        })}          
        ...
        <Box sx={{display:"flex", justifyContent:"center"}}>
          <Button variant="outlined" sx={{mt:2,width:"50%", height:"50px", borderRadius:"50px", color:"black", borderColor:"black"}}>Place Order</Button>
        </Box>
